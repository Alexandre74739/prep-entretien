...
export default function App() {
  ...
  const handleInterviewQuiz = () => {
    setIsInterview(true);
    setActiveId("__interview__");
    if (isMobile) setSidebarOpen(false);
  };
    ...
    <div style={{
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-tab { transition: all 0.18s cubic-bezier(0.4,0,0.2,1); }
        .btn-tab:hover { transform: translateY(-1px); }
        .card-q { transition: border-color 0.2s, box-shadow 0.2s, transform 0.15s; }
        .card-q:hover { transform: translateY(-1px); }
        .opt-btn { transition: all 0.15s ease; }
        .opt-btn:hover:not(:disabled) { transform: translateX(3px); }
        .nav-pill { transition: all 0.15s ease; }
        .nav-pill:hover { transform: translateX(2px); }
        @keyframes fadeIn {
      ...
      <Topbar
        ...
        section={isInterview ? { label: "Quiz Entretien", icon: "🎤", color: "#f59e0b" } : section}
      />
      ...
      <div style={{ display: "flex", height: "calc(100vh - 56px)", overflow: "hidden", position: "relative" }}>
        ...
        {isMobile && sidebarOpen && (
          <div
            ...
            style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(2px)", zIndex: 20 }}
          />
        )}
        ...
        <div style={{
          ...
          transition: "transform 0.28s cubic-bezier(0.4,0,0.2,1), width 0.28s, min-width 0.28s",
          ...
          <Sidebar
            ...
            onInterviewQuiz={handleInterviewQuiz}
          />
        </div>
            ...
            <>
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 24, flexWrap: "wrap" }}>
                ...
                {[
                  ...
                  const active = tab === id;
                  return (
                    <button key={id} className="btn-tab" onClick={() => setTab(id)}
                      style={{
                        padding: "7px 16px", borderRadius: 8,
                        border: active ? `1.5px solid ${acc}` : `1.5px solid ${theme.border}`,
                        background: active ? (darkMode ? acc + "22" : acc + "18") : "transparent",
